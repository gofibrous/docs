...
<AccordionGroup>
  <Accordion title="quote" icon="tag">
    **Category:** Read-Only
    ...
    Fetches a swap price without a wallet or a session. This is the only skill that works before authentication, so an agent can answer a pricing question for a user who has not logged in — and then offer `authenticate-wallet` if they want to execute.
  ...
  <Accordion title="aave" icon="building-columns">
    **Category:** DeFi
    ...
    Manages Aave V3 lending positions on **Base only** — status, markets, supply, borrow, repay, withdraw, and health factor monitoring. The CLI automatically handles ETH ↔ WETH wrapping and unwrapping for `supply`, `repay`, and `withdraw` actions.
    ...
    4. When fully closing a position, always use `max` as the amount. This sends `MAX_UINT256` and prevents dust residuals.
    5. Use `ETH` as the token symbol for ETH-based operations — the CLI handles wrapping/unwrapping automatically.
