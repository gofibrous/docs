...
<AccordionGroup>
  <Accordion title="authenticate-wallet" icon="key">
    **Category:** Auth
    ...
    Manages the wallet session. Supports email OTP login (via Privy server wallets) and private key import. This skill must run before any other wallet operation.
    ...
    3. `auth login` must complete before `auth verify` — they are sequential steps.
    4. After successful authentication, always run `status` to confirm the session.
