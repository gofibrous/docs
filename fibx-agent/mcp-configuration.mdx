...
**Preview output:** The amounts and route with `mode`, plus `estimatedGas` when
it can be calculated safely. For a routed ERC-20 swap, `requiresApproval`
indicates whether approval is needed before live execution.
...
| `chain`     | string | No       | `base`, `citrea`, `hyperevm`, or `monad`     |
| `simulate`  | boolean | No      | Validate or estimate the transfer without submitting it |

**Execution output:** `txHash`, `amount`, `token`, `recipient`, `chain`, and
`explorer` when available.

**Preview output:** The transfer details with `mode`, and `estimatedGas` where
available.
