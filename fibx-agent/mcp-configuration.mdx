...
4. Tools execute against live blockchains. Transactional tools are marked as **destructive**, so compatible editors prompt for confirmation before execution.
...
These tools are marked as **destructive** — the AI editor will ask for confirmation before executing. All three accept `simulate=true` to estimate fees without sending a transaction.
