...
<Warning>
  **API Key Required**: Integrator features (integratorAddress, integratorFeePercentageBps or integratorSurplusPercentageBps) require an API key. Generate and manage your API keys at the [Fibrous Partnership Portal](https://portal.fibrous.finance/). Include the API key in the request headers as `X-API-Key`.
</Warning>
