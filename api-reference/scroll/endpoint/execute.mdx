---
title: 'Execute'
description: 'Execute a swap using the optimal route found through Fibrous on Citrea'
api: 'POST /citrea/execute'
---
...
Execute a token swap using the optimal route previously found through the route endpoint. This endpoint handles the actual transaction execution and ensures the swap is performed with the best possible outcome on Citrea.

## Request Body Parameters

<ParamField body="route" type="object" required>
  The route object returned from the `/citrea/route` endpoint.
