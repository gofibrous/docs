...
```json
{
  ...
  "details": "One or more provided token addresses are not valid or not supported"
}
```
...
```json
{
  ...
  "details": "Not enough liquidity for one or more requested swap amounts"
}
```
...
```json
{
  ...
  "details": "The number of amounts must match the number of input token addresses"
}
```

## Related Endpoints

- [Route](/api-reference/starknet/endpoint/route) - Find single swap route
